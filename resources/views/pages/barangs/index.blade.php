        ...
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('barangs.index') }}" method="GET" class="mb-3">
                        <div class="input-group">
                            <input type="text" name="search" class="form-control"
                                placeholder="Search by nama or deskripsi" value="{{ $search ?? '' }}">
                    ...
                    <a href="{{ route('laporan.stok_barang') }}" class="btn btn-info mb-3" target="_blank">Cetak Laporan
                        Stok Barang</a>
                        ...
                        <table class="table-striped table">
                            ...
                            <tbody>
                                @forelse ($expiringStocks as $key => $item)
                                    <tr
                                        @if ($item['sisa_hari'] < 0) @elseif ($item['sisa_hari'] < 5) class="text-white" @endif>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $item['barang']->nama ?? '-' }}</td>
                                        <td>{{ $item['kadaluarsa'] ? $item['kadaluarsa']->format('d-m-Y') : '-' }}</td>
                                        <td>{{ $item['stok'] ?? '-' }}</td>
                                        <td>{{ isset($item['sisa_hari']) ? $item['sisa_hari'] . ' hari' : '-' }}</td>
                                    </tr>
                                @empty
                                    <tr>
